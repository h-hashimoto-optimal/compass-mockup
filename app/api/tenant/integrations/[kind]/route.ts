...
export async function GET(_req: Request, { params }: { params: { kind: string } }) {
  ...
  if (!checkKind(params.kind)) return NextResponse.json({ error: '未知の連携種別' }, { status: 400 });
  return NextResponse.json({ ...(await getIntegrationStatus(t, params.kind)), masterKey: hasMasterKey() });
}
...
export async function PUT(req: Request, { params }: { params: { kind: string } }) {
  ...
  return NextResponse.json(await saveIntegration(t, params.kind, secrets));
}
...
export async function DELETE(_req: Request, { params }: { params: { kind: string } }) {
  ...
  if (!t) return NextResponse.json({ error: 'テナント文脈がありません' }, { status: 403 });
  if (!checkKind(params.kind)) return NextResponse.json({ error: '未知の連携種別' }, { status: 400 });
  return NextResponse.json({ ok: await disconnectIntegration(t, params.kind) });
}
