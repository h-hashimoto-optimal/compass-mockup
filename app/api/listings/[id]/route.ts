...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const listing = await getTenantListing(tenantId, (await params).id);
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updated = await updateListing(tenantId, (await params).id, body);
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  const tenantId = await currentTenantId();
  if (!tenantId) return NextResponse.json({ error: 'テナント文脈がありません' }, { status: 403 });
  const ok = await softDeleteListing(tenantId, (await params).id);
