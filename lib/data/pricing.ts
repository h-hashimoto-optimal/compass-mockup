...
export type PricingInput = {
  ...
  intlShippingJpy: number; // 重量別の国際配送料（円。HANIRO等）
...
export function computeListingPricing(sourcePriceJpy: number, p: PricingInput): PricingResult {
  ...
  const shipping = p.domesticShippingJpy + p.intlShippingJpy; // 送料合計（国内＋重量別国際）
  const costJpy = sourcePriceJpy + shipping;
  ...
  // 固定した売価に対し、手数料控除後の手取りで賄える仕入上限（円）＝赤字下限（送料分を差し引く）
  const maxCostJpy = (listPrice * (1 - p.sellFeeRate)) / effFx;
  const floorPriceJpy = Math.floor(maxCostJpy - shipping);
